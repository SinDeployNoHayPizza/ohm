...
Expose OHM as a Model Context Protocol (MCP) server so agent clients (Claude Desktop, opencode) can drive it: run single prompts, run autonomous goals, and inspect status, sessions, skills, and models. Stage 1 is a stateless tool surface over stdio and streamable HTTP; streaming, resources, subscriptions, and HTTP auth are deferred to Stage 2.
