...
Structured logging bootstrapped from OHMConfig, a `ohm.metrics.*` registry for session/agent/provider telemetry, and CLI JSON surfaces — never writing logs or metrics to `ohm run` stdout. Stage 1; export and correlation IDs deferred.
